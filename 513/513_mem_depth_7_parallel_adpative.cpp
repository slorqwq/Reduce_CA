...
int unflagged_v(int (*Error_vector)[n], double probability, double Gamma, double beta, double alpha);
int unflagged_m124(int (*Error_vector)[n], double probability, double Gamma, double beta, double alpha);
int unflagged_3m124(int (*Error_vector)[n], double probability, double Gamma, double beta, double alpha);
int unflagged_4m123(int (*Error_vector)[n], double probability, double Gamma, double beta, double alpha);
int unflagged_3m24(int (*Error_vector)[n], double probability, double Gamma, double beta, double alpha);
...
//讀取table    
    ...
    ifstream fin8("table\\adptive_index_p.txt");
    if(!fin8) {
        cout << "無法讀入檔案\n";
        system("pause");
        return 0;
...
//============================================================================================================================    
    ...
    cout << "%[[5,1,3]] parallel^2 with the last round is  parallel adpative, memeory threshold" << " ,loop=" << N-1 << " ,gamma, alpha, beta=" << Gamma << alpha << beta << endl << endl << endl;
...
/*
	...
	cout<< "Error_vector Z " << Error_vector[1][0]<<Error_vector[1][1] <<Error_vector[1][2] <<Error_vector[1][3] <<Error_vector[1][4]  << endl ;
 	...
 	cout<< "m1534f12 " << Error_vector[0][5]<<Error_vector[0][8] <<Error_vector[0][6] <<Error_vector[0][9] <<Error_vector[1][7] <<Error_vector[1][10]  << endl ;   	
	}
*/
	//---------------------------------------------------------------------------------------------------------------------------------------------		
	...
	//  unflagged_v(Error_vector, probability, 0, 0,0 ); //g1,5,3,4
...
/*
	cout<< "flag rise: " << endl;
 	cout<< "Error_vector X " << Error_vector[0][0]<<Error_vector[0][1] <<Error_vector[0][2] <<Error_vector[0][3] <<Error_vector[0][4]  << endl ;
	cout<< "Error_vector Z " << Error_vector[1][0]<<Error_vector[1][1] <<Error_vector[1][2] <<Error_vector[1][3] <<Error_vector[1][4]  << endl ;
	cout<< "m1534f12m'1534  " << Error_vector[0][5]<<Error_vector[0][8] <<Error_vector[0][6] <<Error_vector[0][9] <<Error_vector[1][7] <<Error_vector[1][10]  << Error_vector[0][15]<<Error_vector[0][16] <<Error_vector[0][17] <<Error_vector[0][18] << endl ;   	
	cout<< "symdorme_result " << symdorme_result2[0]<<symdorme_result2[1] <<symdorme_result2[2] <<symdorme_result2[3] << symdorme_result2[4]<<symdorme_result2[5] <<symdorme_result2[6] <<symdorme_result2[7] <<symdorme_result2[8] <<symdorme_result2[9] << endl  << endl ;   	
*/
	 ...
	 //compare test2 and symdorme_result array and get index j
            ...
            */  
                ...
                if (compare_result == 0) {
					...
					if ( adptive_index[j][0] == 2 && adptive_index[j][1] == 2 && adptive_index[j][2] == 2 && adptive_index[j][3] == 2  ){
                        unflagged_v(Error_vector, probability, Gamma, beta, alpha );
                        }
                    if ( adptive_index[j][0] == 2 && adptive_index[j][1] == 2 && adptive_index[j][2] == 0 && adptive_index[j][3] == 2  ){
                        unflagged_m124(Error_vector, probability, Gamma, beta, alpha );
                    }
                    if ( adptive_index[j][0] == 1 && adptive_index[j][1] == 1 && adptive_index[j][2] == 1 && adptive_index[j][3] == 1  ){
                        Error_vector[0][15] = 1;  // g'1
						Error_vector[0][16] = 1;  // g'5
						Error_vector[0][17] = 1;  // g'3
						Error_vector[0][18] = 1;  // g'4                    
                    }
                    if ( adptive_index[j][0] == 2 && adptive_index[j][1] == 2 && adptive_index[j][2] == 1 && adptive_index[j][3] == 2  ){
                        unflagged_3m124(Error_vector, probability, Gamma, beta, alpha );
                    }
                    if ( adptive_index[j][0] == 1 && adptive_index[j][1] == 0 && adptive_index[j][2] == 0 && adptive_index[j][3] == 0  ){
                        Error_vector[0][15] = 1;  // g'1
						Error_vector[0][16] = 0;  // g'5
						Error_vector[0][17] = 0;  // g'3
						Error_vector[0][18] = 0;  // g'4   
                    }
                    if ( adptive_index[j][0] == 2 && adptive_index[j][1] == 2 && adptive_index[j][2] == 2 && adptive_index[j][3] == 1  ){
                        unflagged_4m123(Error_vector, probability, Gamma, beta, alpha );
                    }
                    if ( adptive_index[j][0] == 1 && adptive_index[j][1] == 0 && adptive_index[j][2] == 1 && adptive_index[j][3] == 1  ){
                        Error_vector[0][15] = 1;  // g'1
						Error_vector[0][16] = 0;  // g'5
						Error_vector[0][17] = 1;  // g'3
						Error_vector[0][18] = 1;  // g'4  
                    ...
                    if ( adptive_index[j][0] == 0 && adptive_index[j][1] == 2 && adptive_index[j][2] == 1 && adptive_index[j][3] == 2  ){
                        unflagged_3m24(Error_vector, probability, Gamma, beta, alpha );
                    }                   
                    
                    if ( adptive_index[j][0] == 0 && adptive_index[j][1] == 0 && adptive_index[j][2] == 1 && adptive_index[j][3] == 0  ){
                        Error_vector[0][15] = 0;  // g'1
						Error_vector[0][16] = 0;  // g'5
						Error_vector[0][17] = 1;  // g'3
						Error_vector[0][18] = 0;  // g'4  
...
//---circuit--g1,5,3,4--------------------------------------------------------------------------------------------------
/*        
	circuit(Error_vector, probability, Gamma, beta );	
	//loading syndorme result to array	 
		
			symdorme_result[0] = Error_vector[0][5]; // m1 = g1
			symdorme_result[1] = Error_vector[0][8]; // m5 = g5
			symdorme_result[2] = Error_vector[0][6]; // m3 = g3
			symdorme_result[3] = Error_vector[0][9]; // m4 = g4
...
//---unflagged_v circuit--g1,5,3,4--------------------------------------------------------------------------------------------------

/*	
	unflagged_v(Error_vector, probability, Gamma, beta );	
...
//---unflagged_s circuit--g1,2,3,4--------------------------------------------------------------------------------------------------

/*	
	unflagged_s(Error_vector, probability, Gamma, beta );	
    

	//loading syndorme result to array	 

			symdorme_result[0] = Error_vector[0][5];
			symdorme_result[1] = Error_vector[0][6];
			symdorme_result[2] = Error_vector[0][7];
			symdorme_result[3] = Error_vector[0][8];
